...
export const signIn = async (request, response, next) => {
    try {
        ...
        if (customer.length) {
            ...
            console.log(request.body.password,customer[0].password);
...
export const updataProfile = (request, response, next) => {
    Customer.updateOne({ contact: request.body.contact},{$set:{photo:request.body.photo,customerName:request.body.customerName ,email:request.body.email}})
        .then(result => {
            console.log(result);
