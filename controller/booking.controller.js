...
// import sequalize from "../model/dbconfig.js";
...
export const updateStatus=async(request,response,next)=>{
    try{
   let status= await Booking.findOne({_id:request.body._id});
   console.log(status);
...
export const addMechanic=async(request,response,next)=>{
    try{
   let status= await Booking.findOne({_id:request.body.bookingId
   });
   console.log();
//    console.log();
...
export const bill=async(request,response,next)=>{
    try{
   let status= await Booking.findOne({
        _id:request.body.bookingId,
   });
   ...
   if(status.status=="accept"){
        let update=await Booking.updateOne({
            _id:request.body.bookingId
                ...
                billAmmount:request.body.billAmmount,
                actualProblem:request.body.actualProblem,
      })       
           response.status(200).json({update:update,status:true});
       }
...
export const id=async (request,response,next)=>{
    try{
 let data=await Booking.findById(
        request.body.bookingId
 )
   response.status(200).json({data:data,status:true});
response.status(401).json({data:"bed request",status:false});
}
 catch(err){
    console.log(err);   
